...
export default () => {
  ...
  let color = wuiPaletteColorBlind({ rotations: 2, order: 'group' }).slice(
    18,
    20
  );
  if (formatted) {
    color = [
      wuiPaletteForTemperature()[0],
      wuiPaletteGray(5)[isDarkTheme ? 4 : 0],
    ];
  }
  ...
  if (formatted && formattedData) {
    data = ordered
      ...
      : DAYS_OF_RAIN;
    usesRainData = true;
    color = wuiPaletteForTemperature(3);
  } else {
  ...
  <BarSeries
    ...
    color={${
      formatted
        ? '[wuiPaletteForTemperature()[0], wuiPaletteGray(5)[isDarkTheme ? 4 : 0]]'
        : "wuiPaletteColorBlind({ rotations: 2, order: 'group' }).slice(18, 20)"
    }}
  />
  <Axis
    id="bottom-axis"
