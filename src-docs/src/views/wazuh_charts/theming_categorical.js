...
export const Categorical = () => {
  ...
  const highlightColor = wuiPaletteColorBlind()[2];
  ...
  const createQuantityChart = numCharts => {
    const vizColors = wuiPalettePositive(numCharts);
    ...
    setVizColorsString(`wuiPaletteCool(${numCharts})`);
    setChartType('BarSeries');
  };

  const createTrendChart = numCharts => {
    const vizColors = wuiPaletteForStatus(numCharts);
    ...
    setVizColors(vizColors);
    setVizColorsString(`wuiPaletteForStatus(${numCharts})`);
    setChartType('BarSeries');
  };

  const createHighlightChart = numCharts => {
    const vizColors = wuiPaletteGray(numCharts);
    ...
    setVizColorsString(
      `wuiPaletteGray(${numCharts})[length - 1] = highlightColor`
    );
    setChartType('LineSeries');
  };
  ...
  const customTitle =
    colorType === 'Highlight' ? (
      <WuiTitle size="xxs">
        <h4>
          <WuiIcon type="dot" color={highlightColor} /> My number of issues
          compared to others
        </h4>
      </WuiTitle>
    ) : (
    ...
    for (let index = 0; index < 4; index++) {
      ...
      const chart = (
        <ChartType
          ...
          xAccessor={'x'}
          yAccessors={['y']}
          color={[wuiPaletteColorBlind()[index < 2 ? 0 : 1]]}
          lineSeriesStyle={{
            line: {
