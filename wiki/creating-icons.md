...
WUI provides an ever-growing set of [icons][icons], but our set can be incomplete. If you find you need an icon that does not exist, create a new issue and tag it with the *icons* label. A designer from the WUI team will respond to discuss your needs.

If you are willing and able to design the icon yourself, this document describes the guidelines for designing a new icon, cleaning up the SVG, and getting it added to WUI. While designers on the WUI team are available to assist, we greatly appreciate your contributions and pull requests.

If you read through these guidelines or begin designing your icon and realize you're in too deep, then create an issue in this repo and request assistance. An WUI team member will reply and discuss options.

_**Note**: The `WuiIcon` component accepts external references to icon files, so you have the option to maintain the icon in your consuming application._
...
Lastly, we reserve the right to reject any icons that do not fit the WUI style or may be deemed inappropriate.
...
#### _For Sketch users_
_As a reference, you can download and view the `icons.sketch` file via the **Sketch libraries** link atop the [WUI documentation site][docs] site. The Symbols page within this file contains artboards for the vast majority of WUI glyphs._
...
## Add the icon to the WUI repo

Once you've designed your new icon, the last step is adding it to the WUI repo.

### Clean the SVG

WUI provides SVG icon formats only. After exporting your icon as an SVG from your design tool, open it in a code or text editor and remove any unnecessary elements and attributes, such as:
- `<def>...</def>`
- `<use>...</use>`
- `id: <value>`
- `fill: <value>`
...
_1. Add your glyph to the `WuiIcon` component_
...
Opening a PR will notify the WUI team that your work is ready for review. Please include a screenshot in the description and reference the issue that your PR fixes.

### Ship it
Once your PR is approved, you will be able to merge it and give yourself a well-deserved pat on the back. Finally, stay tuned for the next release of WUI at which point your icon will become available to the masses and appear on the WUI docs site.

:trophy: _**Welcome to the Official WUI Icon Design Club**_ :beers:
...
_\** Run `yarn && yarn start` to view the WUI docs site locally._


[icons]: https://wazuh.github.io/wui/#/display/icons
