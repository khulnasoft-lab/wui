# Using react-router with WUI

WUI doesn't prescribe the use of any particular routing library, and we also don't want to incur
the maintenance burden of supporting router-specific components. For these reasons, WUI doesn't
publish any tools for working with `react-router` (or any other routing lib). However,
integrating WUI with `react-router` on the consumer's side is fairly straightforward.
...
open the link in a new tab. Any mechanism for integrating WUI with `react-router` needs to bridge
this `to` prop with WUI components' `href` and `onClick` props.
...
There are many techniques for integrating WUI with `react-router` ([see below](#techniques-we-dont-recommend) for some techniques we don't recommend), but we think these two are the strongest:

### 1) Conversion function (recommended)

You can use a conversion function to convert a `to` value
to `href` and `onClick` values, which you can then pass to any WUI button or link component.
Many WUI components are designed to accept both props if they accept one.
...
```jsx
<WuiLink {...getRouterLinkProps('/location')}>Link</WuiLink>
```
...
<RouterLinkAdapter to="/location">
  {(onClick, href) => <WuiLink onClick={onClick} href={href}>Link</WuiLink>}
<RouterLinkAdapter/>
```
...
conversion function (called `getRouterLinkProps` here). The WUI documentation site [uses this approach](../src-docs/src/services/routing/routing.js).
...
In react-router 5.1, we can fully capitalize in the React Hooks utility, in this case, `useHistory`. Using this, we do not need other HOC wrapper files and global router variable. We just need to create the file below, and then use it anywhere by importing `WuiCustomLink`.

```jsx
// File name: "WuiCustomLink.js".
import React from 'react';
import { WuiLink } from '@wazuh/wui';
...
export default function WuiCustomLink({ to, ...rest }) {
  // This is the key!
...
### Using WUI classes with the react-router `<Link>` component

It's possible to integrate WUI with `react-router` by using its CSS classes only:

```jsx
<Link className="wuiLink" to="/location">Link</Link>
```

But it's important to be aware of two caveats to this approach:

* WUI's components contain a lot of useful behavior. For example, `WuiLink` will render either
  ...
  features if they use WUI's CSS instead of its React components.
* This creates a brittle dependency upon the `wuiLink` CSS class. If we were to rename this
  class in WUI, this would constitute a breaking change and we'd make a note of it in the change
